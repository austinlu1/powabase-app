/**
 * POST /api/session-sources/attach-url
 * Body: { url: string }
 * Imports a URL to Powabase for extraction, fetches the markdown,
 * deletes the source, and returns the extracted text.
 */
import { NextRequest, NextResponse } from "next/server";
import { getUserFromCookie, pbGet, pbDelete, POWABASE_URL, powabaseHeaders } from "@/lib/powabase-server";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Snapshot existing source IDs for diff fallback
    ...
    if (!sourceId) {
      return NextResponse.json({ error: "Could not identify imported source" }, { status: 502 });
    }

    await waitForExtraction(sourceId);
    const extractedText = await getExtractedText(sourceId);

    // Delete source — temporary extraction vehicle only
